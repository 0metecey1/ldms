...
static int lcounter_new(lua_State *L)
{
     ...
     * pre-initialize
     * the member of the userdata in case initialization fails in
     * some way. If
     * that happens we want the userdata to be in a
     * consistent state for __gc. */
    cu       = (lcounter_userdata_t
            *)lua_newuserdata(L, sizeof(*cu));
    cu->c    = NULL;
    cu->name = NULL;

    /* Add the metatable to the stack. */
    luaL_getmetatable(L, "LCounter");
    /* Set the metatable on the
     * userdata. */
...
int luaopen_lcounter(lua_State *L)
{
    ...
    /* Register the object.func functions
     * into the table that is at the top
     * of the
     * stack. */
    luaL_newlib(L,
            lcounter_functions);
